function [prob_term_topic, prob_topic_doc, lls,time,Terror,iter] = ARTMsparse(termDocMatrix,numTopic,iter)
tstart=tic;
time=zeros(10000,1);
Terror=time;
...
for i = 1 : iter
	...
	for d = 1:numDoc
		w = find(termDocMatrix(:, d));
		for z = 1:numTopic
 			prob_topic_doc(z, d) = max(sum(termDocMatrix(w, d) .* prob_topic_term_doc{z}(w, d))-0.2/numTopic,0);
%             prob_topic_doc(z, d) = max(sum(termDocMatrix(w, d) .* prob_topic_term_doc{z}(w, d)),0);
	...
	for z = 1:numTopic
		for w = 1:numTerm
			...
			prob_word_topic(w, z) = max(sum(termDocMatrix(w, d) .* prob_topic_term_doc{z}(w, d))-0.2/numTerm,0);
